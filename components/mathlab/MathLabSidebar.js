...
import { isMathLabAdminUser, isSiteAdminUser } from "@/lib/auth/productAdmins";
...
export default function MathLabSidebar() {
  ...
  const isSiteAdmin = Boolean(userData && user && isSiteAdminUser(userData, user.email));
  const isMathLabAdmin = Boolean(userData && user && isMathLabAdminUser(userData, user.email));
  ...
  } else if (isMathLabAdmin) {
    // Math Lab admins: student view, tutor dashboard, history, tracking, team panel
