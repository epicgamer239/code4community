...
export function sessionSortTimestampMs(session) {
  if (session?.sessionType === "IN_PERSON") {
    const end = firestoreToDate(session.sessionEndTime);
    if (end) return end.getTime();
    const start = firestoreToDate(session.sessionStartTime);
...
/** Date-only label (e.g. admin-assigned mini lessons). */
export function formatSessionDateOnly(value, fallback = "—") {
  const d = firestoreToDate(value);
  if (!d) return fallback;
...
function formatSameDayTimeRange(start, end) {
  const datePart = formatSessionDateOnly(start);
  const opts = { hour: "numeric", minute: "2-digit", hour12: true };
  const startFull = start.toLocaleTimeString([], opts);
  const endFull = end.toLocaleTimeString([], opts);
  const startMer = startFull.slice(-2);
  const endMer = endFull.slice(-2);
  const startTime = startFull.replace(/ (AM|PM)$/i, "");
  const endTime = endFull.replace(/ (AM|PM)$/i, "");
  if (startMer === endMer) {
    return `${datePart}, ${startTime}–${endTime} ${startMer}`;
  }
  return `${datePart}, ${startFull}–${endFull}`;
}

/**
 * Writing Center list column: in-person uses tutor Start/End clicks, not request createdAt.
 * @param {{ sessionType?: string, sessionStartTime?: unknown, sessionEndTime?: unknown, createdAt?: unknown } | null | undefined} session
 */
export function formatWritingCenterSessionDateTime(session, fallback = "—") {
  ...
  if (session.sessionType === "IN_PERSON") {
    ...
    if (start) {
      return `${formatSessionDate(start)} – …`;
    }
    return "Not started";
  }

  return formatSessionDate(session.createdAt, fallback);
}
