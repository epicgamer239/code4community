...
import { isLegacyMathLabAdminRole } from "@/lib/auth/productAdmins";
import { normalizeEmail } from "@/lib/email";
import { getTutorServices, hasAnyTutorService } from "@/lib/tutorServices";

/** @typedef {{ id: string, email?: string, displayName?: string, role?: string, mathLabRole?: string, mathLabAdmin?: boolean, writingCenterRole?: string, services?: string[], pending?: boolean }} TeamUser */
...
export function classifyTeamUser(user) {
  ...
  if (user.mathLabAdmin === true || isLegacyMathLabAdminRole(user)) return "appointed-admin";
