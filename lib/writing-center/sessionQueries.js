...
export async function fetchTutorWritingCenterHistory(firestore, tutorId) {
  ...
  const statusFilter = where("status", "in", WC_HISTORY_STATUSES);
  const [byTutorIdSnap, byTutorIdsSnap] = await Promise.all([
    getDocs(
      query(
        coll,
  ...
  const map = new Map();
  for (const snap of [byTutorIdSnap, byTutorIdsSnap]) {
...
/**
 * One-shot completed mini lessons assigned to a teacher for tutor rating upload.
 * @param {import('firebase/firestore').Firestore} firestore
 * @param {string} teacherId
 */
export async function fetchTeacherMiniLessons(firestore, teacherId) {
  if (!teacherId) return [];
  const snap = await getDocs(
    query(
      collection(firestore, "sessions"),
      where("teacherId", "==", teacherId),
      where("sessionType", "==", "MINI_LESSON"),
